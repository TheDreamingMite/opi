...
#define RESET_GPIO 360 // PL8 on Orange Pi 3 LTS
...
// NEW: Define CS GPIO configuration
#define CS_GPIO 227    // PH3 on Orange Pi 3 LTS
#define CS_GPIO_PATH "/sys/class/gpio/gpio227/value"
#define CS_GPIO_DIR_PATH "/sys/class/gpio/gpio227/direction"
#define CS_GPIO_EXPORT_PATH "/sys/class/gpio/export"


// --- Register definitions (from loragw_reg.h/.c) ---
#define SX1302_REG_COMMON_BASE_ADDR 0x5600
#define SX1302_VERSION_REG_ADDR 0x5606 // SX1302_REG_COMMON_BASE_ADDR + 6
...
int main() {
    ...
    // NEW: Configure CS GPIO
    print_timestamp();
    fprintf(stdout, "[INFO] 4a. Configuring GPIO %d (CS)...\n", CS_GPIO);
    if (access(CS_GPIO_PATH, F_OK) == -1) {
        if (gpio_export(CS_GPIO) != 0) {
    ...
    if (gpio_set_direction(CS_GPIO, "out") != 0) {
    ...
    // Read initial CS state (optional, for info)
    int fd_cs_val = open(CS_GPIO_PATH, O_RDONLY);
    if (fd_cs_val >= 0) {
        char val_char;
        if (read(fd_cs_val, &val_char, 1) > 0) {
            print_timestamp();
            fprintf(stdout, "[INFO] Initial CS pin value: %c\n", val_char);
        }
        close(fd_cs_val);
    }
    ...
    print_timestamp();
    fprintf(stdout, "[INFO] 4b. Configuring GPIO %d (RESET)...\n", RESET_GPIO);
    ...
    // Read initial CS state again (optional, for info)
    fd_cs_val = open(CS_GPIO_PATH, O_RDONLY);
    if (fd_cs_val >= 0) {
        char val_char;
        if (read(fd_cs_val, &val_char, 1) > 0) {
            print_timestamp();
            fprintf(stdout, "[INFO] CS pin value before SPI transaction: %c\n", val_char);
        }
        close(fd_cs_val);
    }
    ...
    if (chip_responded) {
        ...
        print_timestamp();
        fprintf(stdout, "  - CS wire not connected to correct pin (PH3/Pin 24, GPIO 227) (common cause of 0x00)\n"); // Updated text
        print_timestamp();
    ...
    return (chip_responded && chip_version == 0x01) ? 0 : 1;
}
